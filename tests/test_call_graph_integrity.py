...
def call_graph_content() -> str:
    ...
    assert CALL_GRAPH_PATH.exists(), f"Le fichier de documentation {CALL_GRAPH_PATH} est introuvable."
...
def test_documented_files_exist(call_graph_content: str):
    ...
    assert start_marker in call_graph_content, f"Section '{start_marker}' introuvable dans call_graph.md."
    ...
    assert len(matches) >= 20, f"Trop peu de modules extraits ({len(matches)}). Vérifier le format du tableau."

    for file_rel_path in matches:
        file_path = root / file_rel_path.strip()
        assert file_path.exists(), f"Le fichier '{file_rel_path}' documenté dans call_graph.md n'existe pas sur disque."
...
def test_documented_functions_exist():
    ...
    for module_name, func_names in checks:
        ...
        for fn in func_names:
            assert hasattr(mod, fn), f"La fonction '{fn}' n'existe pas dans le module '{module_name}'."
            attr = getattr(mod, fn)
...
def test_cli_options_documented(call_graph_content: str):
    ...
    # Extraction des actions du parser
    from cptcopro import main as main_mod
    # Analyser la fonction _parse_cli_args via son parser
...
def test_all_streamlit_pages_documented(call_graph_content: str):
    ...
    assert len(page_files) == 14, f"Nombre inattendu de pages dans Pages/ : {len(page_files)} (14 attendues)."

    for pf in page_files:
        assert pf in call_graph_content, f"La page Streamlit '{pf}' n'est pas mentionnée dans call_graph.md."
