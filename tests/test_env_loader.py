    ...
    def test_returns_exe_dir_in_frozen_mode(self, tmp_path):
        ...
        with patch('sys.frozen', True, create=True):
            with patch('sys.executable', str(fake_exe)):
                ...
                with patch.object(env_loader, 'sys') as mock_sys:
                    mock_sys.frozen = True
                    mock_sys.executable = str(fake_exe)
                    # Appeler directement le code
                    if getattr(mock_sys, 'frozen', False):
                        result = Path(mock_sys.executable).parent
...
class TestLoadAndValidateEnv:
    ...
    def test_raises_value_error_when_vars_missing(self, tmp_path):
        ...
        env_file = tmp_path / ".env"
        env_file.write_text("login_site_copro=user\n")  # Manque password et url
        
        with patch('cptcopro.utils.env_loader.get_env_file_path', return_value=env_file):
            with patch.dict(os.environ, {"login_site_copro": "user"}, clear=True):
    ...
    def test_returns_env_dict_when_valid(self, tmp_path):
        ...
        env_file.write_text(
            ...
            "url_site_copro=http://example.com\n"
        )
        ...
        with patch('cptcopro.utils.env_loader.get_env_file_path', return_value=env_file):
            with patch.dict(os.environ, {
                ...
                assert result["password_site_copro"] == "pass"
                assert result["url_site_copro"] == "http://example.com"
    
    def test_accepts_custom_required_vars(self, tmp_path):
        """Accepte une liste personnalisée de variables requises."""
        env_file = tmp_path / ".env"
        env_file.write_text("CUSTOM_VAR=value\n")
        
        with patch('cptcopro.utils.env_loader.get_env_file_path', return_value=env_file):
            with patch.dict(os.environ, {"CUSTOM_VAR": "value"}):
...
class TestGetCredentials:
    ...
    def test_returns_tuple_when_valid(self, tmp_path):
        ...
        env_file = tmp_path / ".env"
        env_file.write_text(
            "login_site_copro=myuser\n"
            "password_site_copro=mypass\n"
            "url_site_copro=https://site.com\n"
        )
