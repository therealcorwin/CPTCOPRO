...
def setup_db(path: Path | None = None) -> None:
    dbmod.integrite_db()


def test_relance_tables_created():
    dbmod.integrite_db()
...
def test_due_frequency_logic():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                "VALUES (%s, %s, %s, 0.0, '2026-01-01', CURRENT_DATE)",
                ("D001", "Dupont", 3000.0),
            )

    dbmod.update_relance_config(frequency_days=14)

    rows = dbmod.list_relances_due()
    assert len(rows) == 1
    assert rows[0]["due"] == 1
    ...
    rows_after = dbmod.list_relances_due()
...
def test_create_update_delete_template():
    ...
    templates = dbmod.list_relance_templates()
    assert len(templates) == 2

    dbmod.update_relance_template(template_id, is_default=True)
    updated = dbmod.get_relance_template(template_id)
    assert updated["is_default"] == 1

    default_templates = [t for t in dbmod.list_relance_templates() if t["is_default"]]
    assert len(default_templates) == 1

    assert dbmod.delete_relance_template(template_id) is True
    remaining = dbmod.list_relance_templates()
    assert len(remaining) == 1


def test_delete_last_template_is_refused():
    dbmod.integrite_db()
    templates = dbmod.list_relance_templates()
    only_id = templates[0]["template_id"]

    with pytest.raises(ValueError):
        dbmod.delete_relance_template(only_id)
...
def test_relances_tracking_summary_and_due_metrics():
    dbmod.integrite_db()
    from cptcopro.Database.connection import get_db_connection
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            # Insérer une charge au-dessus du seuil 4p (2800) ou modifier config_alerte
            # Ou insérer charge avec debit=3000
            cur.execute(
                "INSERT INTO charge (code_proprietaire, nom_proprietaire, debit, credit, date, last_check) "
                "VALUES (%s, %s, %s, 0.0, '2026-02-01', CURRENT_DATE)",
                ("D002", "Durand", 3000.0),
            )
    ...
    summary = dbmod.get_relances_tracking_summary()
    ...
    assert float(item["debit_actuel"]) == 3000.0

    due_list = dbmod.list_relances_due()
...
def test_generer_brouillons_relances_automation():
    ...
    # Insérer un copropriétaire avec email de contact et solde débiteur
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO coproprietaires (nom_proprietaire, code_proprietaire, num_apt, type_apt, last_check) "
                "VALUES (%s, %s, %s, %s, CURRENT_DATE) ON DUPLICATE KEY UPDATE type_apt=VALUES(type_apt)",
                ("Martin Auto", "M_AUTO_01", "10", "4p"),
            )
            cur.execute(
                "INSERT INTO charge (code_proprietaire, nom_proprietaire, debit, credit, date, last_check) "
                "VALUES (%s, %s, %s, 0.0, '2026-02-01', CURRENT_DATE) "
                "ON DUPLICATE KEY UPDATE debit=VALUES(debit)",
                ("M_AUTO_01", "Martin Auto", 3500.0),
            )

    dbmod.upsert_relance_destinataire(
        code_proprietaire="M_AUTO_01",
        nom_proprietaire="Martin Auto",
        email_to="martin@example.com",
    )

    # Exécution de la génération automatique en mode template (sans IMAP ni API externe)
    res = generer_brouillons_relances(deposer_imap=False, force_llm=False)

    assert res["generes"] >= 1
    assert len(res["draft_ids"]) >= 1
    assert res["deposes_imap"] == 0

    drafts = dbmod.get_relance_drafts(code_proprietaire="M_AUTO_01")
    assert len(drafts) >= 1
    assert drafts[0]["email_to"] == "martin@example.com"
    assert drafts[0]["status"] == "draft_local"
    assert "Martin Auto" in drafts[0]["subject"] or "charges" in drafts[0]["subject"].lower()

    # Test quand relances désactivées
    dbmod.update_relance_config(enabled=0)
    res_disabled = generer_brouillons_relances(deposer_imap=False)
    assert res_disabled["generes"] == 0
    assert "désactivées" in res_disabled["erreurs"][0].lower()
