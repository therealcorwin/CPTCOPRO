...
def load_all_charges_data(db_path: Path) -> pd.DataFrame:
    ...
    df = df.rename(
        columns={"nom_proprietaire": "proprietaire", "code_proprietaire": "code"}
    )
...
if proprietaire_input:
    ...
    if options:
        ...
        info_df = (
            info_df[["proprietaire", "code", "num_apt", "type_apt"]]
            .drop_duplicates()
            .reset_index(drop=True)
        )
        st.dataframe(appliquer_confidentialite(info_df))
...
if proprietaires_selection:
    ...
    filtered_charges_df = charges_df[
        charges_df["proprietaire"].isin(proprietaires_selection)
    ]
    ...
    if filtered_charges_df.empty:
        ...
        charges_df_sorted = filtered_charges_df.sort_values(
            ["proprietaire", "date"], ascending=[True, True]
        )
        agg = (
            charges_df_sorted.groupby(
                [charges_df_sorted["date"].dt.date, "proprietaire"]
            )
            .agg({"debit": "sum"})
            .reset_index()
        )
        ...
        fig.update_layout(
            title="Évolution du débit par date", xaxis_title="Date", yaxis_title="Débit"
        )
